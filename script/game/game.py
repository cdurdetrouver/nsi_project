...
class Game():
    ...
    def __init__(self, size, title, mdp):
        self.size = size
        # creer la fenetre avec des dimensions donnees
        ...
        # police d'ecriture
        pygame.font.init()
        self.police = pygame.font.SysFont(
            "font/font.ttf", 256)  # police d'ecriture
    ...
    def new_screen(self, screen_size):
        ...
        self.background = pygame.transform.scale(
            self.background, (self.background_size[0] * aug, screen_size[1]))  # redimension
        self.background_blur = pygame.transform.scale(
            self.background_blur, (self.background_size[0] * aug, screen_size[1]))  # redimension
        # taille du fond d'ecran apres redimensionnement
        self.background_size = self.background.get_size()

        self.game_over_image = pygame.image.load(
            "image/game_over.png")  # image de game over
        self.game_over_image = pygame.transform.scale(self.game_over_image, (
            self.size[0] * 50/100, self.size[0] * 50/100 * self.game_over_image.get_size()[1]/self.game_over_image.get_size()[0]))  # redimension

        self.window.blit(
            self.background, (0 - (self.background_size[0] - screen_size[0]), 0))
        connexion = self.police.render(
            "Please Wait ...", True, (255, 255, 255))
        connexion = pygame.transform.scale(connexion, (screen_size[1] * 30/100 * connexion.get_size()[
                                           0] / connexion.get_size()[1], screen_size[1] * 30/100))
        self.window.blit(connexion, (screen_size[0]/2 - connexion.get_size()[
                         0]/2, screen_size[1]/2 - connexion.get_size()[1]/2))

        self.input_rect = pygame.Rect(
            (self.size[0] * 5 / 100, self.size[1]//2), (self.size[0] * 9 / 100, self.size[0] * 3 / 100))
        self.text_pseudo = self.police.render(
            "Pseudo :", True, (255, 255, 255))
        self.text_pseudo = pygame.transform.scale(self.text_pseudo, (self.size[1] * 5 / 100 * self.text_pseudo.get_size()[
                                                  0] / self.text_pseudo.get_size()[1], self.size[1] * 5 / 100))
        self.text_wrong = self.police.render(
            "Pseudo interdit", True, (255, 0, 0))
        self.text_wrong = pygame.transform.scale(self.text_wrong, (self.size[1] * 5 / 100 * self.text_wrong.get_size()[
                                                 0] / self.text_wrong.get_size()[1], self.size[1] * 5 / 100))

        # appel des objets
        # creation de l'interface utilisateur
        self.UI = UI(self.size, self, None)
    ...
    def game_over(self):
        ...
        self.classement.update_classement(
            self.user_name, self.score.get())  # mettre a jour le classement
    ...
    def update(self):
        ...
        if self.menu:
            ...
            self.window.blit(self.background_blur,
                             (0 - (self.background_size[0] - self.size[0]), 0))
            # afficher le score + le boutton jouer
            self.UI.afficher(self.window, "menu")
            # draw rectangle and argument passed which should
            # be on screen
            pygame.draw.rect(self.window, self.color, self.input_rect)
            # render at position stated in arguments
            self.window.blit(self.text_surface,
                             (self.size[0] * 5 / 100, self.size[1]//2))
            self.window.blit(
                self.text_pseudo, (self.size[0] * 5 / 100, self.size[1]//2 - 1.5 * self.text_surface.get_size()[1]))
            if self.wrong_pseudo:
                self.window.blit(
                    self.text_wrong, (self.size[0] * 5 / 100, self.size[1]//2 + 1.5 * self.text_surface.get_size()[1]))
            ...
            self.UI.afficher(self.window, "parameter")
        elif self.gameOver:
            self.window.fill((0, 0, 0))
            self.window.blit(self.game_over_image,
                             (self.size[0] * 25/100, self.size[1] * 25/100))
            ...
            # afficher le background
            self.window.blit(
                self.background, (0 - (self.background_size[0] - self.size[0]), 0))
    ...
    def start_game(self):
        ...
        for i in range(3, 0, -1):
            # image du decompte ("texte a afficher", couleur?, couleur)
            self.image_text = self.police.render(f"{i}", True, (255, 255, 255))
            self.image_text = pygame.transform.scale(self.image_text, (self.size[1] * 40/100 * self.image_text.get_size()[
                                                     0] / self.image_text.get_size()[1], self.size[1] * 40/100))  # redimensionner l'image
            # afficher le background
            self.window.blit(
                self.background, (0 - (self.background_size[0] - self.size[0]), 0))
            self.window.blit(self.image_text, (self.size[0]/2 - self.image_text.get_size()[
                             0]/2, self.size[1]/2 - self.image_text.get_size()[1]/2))
            pygame.display.flip()
            pygame.time.wait(1000)

        # appel la fusee
        self.fusee = Fusee(
            (self.size[0]/2, self.size[1]), self.size, self, self.score)
        self.life = self.fusee.get_life()
        # appel l'interface utilisateur
        self.UI = UI(self.size, self, self.life, self.score)
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.text_surface = self.police.render(
                self.user_name, True, (255, 255, 255))
            self.text_surface = pygame.transform.scale(
                self.text_surface, (self.input_rect.h * self.text_surface.get_size()[0]/self.text_surface.get_size()[1], self.input_rect.h))

            # set width of textfield so that text cannot get
            # outside of user's text input
            self.input_rect.w = max(
                self.size[0] * 6 / 100, self.text_surface.get_width() + 10)
                ...
                if event.type == pygame.KEYDOWN:
                    if active:
                        ...
                        # formation
                            ...
                            self.user_name_2 = unidecode.unidecode(
