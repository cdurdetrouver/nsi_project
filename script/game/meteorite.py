...
class Meteorite(pygame.sprite.Sprite):
    ...
    def __init__(self, vitesse, screen_size):
        ...
        self.image = pygame.transform.scale(
            self.image, (self.screen_size[0] * 0.085, self.screen_size[0] * 0.085 * self.image.get_size()[1]/self.image.get_size()[0]))
        ...
        if self.alpha < 0:
            self.collide_rect = pygame.rect.Rect(self.rect.y + self.size[1]//2, self.rect.x, self.size[1]//2, self.size[0])
        elif self.alpha == 0:
            self.collide_rect = pygame.rect.Rect(self.rect.y + self.size[1]//2, self.rect.x, self.size[1]//2, self.size[0])
        else:
            self.collide_rect = pygame.rect.Rect(self.rect.y + self.size[1]//2, self.rect.x + self.rect.width - self.size[0], self.size[1]//2, self.size[0])
...
class Pluie_Meteorite():
    ...
    def spawn_meteor(self):
        meteor = Meteorite(self.screen_size[1] // 60 // random.uniform(
            self.vitesse_minimum, self.vitesse_maximum), self.screen_size)
    ...
    def afficher(self, screen):
        ...
        for meteor in self.all_meteor:
            ...
            if meteor.delete:
                ...
                new_meteor = Meteorite(self.screen_size[1] // 60 // random.uniform(
                    self.vitesse_minimum, self.vitesse_maximum), self.screen_size)
        ...
        if self.all_meteor.__len__() < 5 and time - self.time_2 > 2000:
                new_meteor = Meteorite(self.screen_size[1] // 60 // random.uniform(
                    self.vitesse_minimum, self.vitesse_maximum), self.screen_size)
                self.all_meteor.add(new_meteor)
                self.time_2 = pygame.time.get_ticks()
