...
class Fusee():
    ...
    def __init__(self, pos, screen_size, game, score):
        ...
        # charger l'image de la fusee
        self.sprite = pygame.image.load("image/fusee.png")
        self.image_size = self.sprite.get_size()  # taille de l'image
        self.sprite = pygame.transform.scale(
            self.sprite, (self.screen_size[0] * 0.085, self.screen_size[0] * 0.085 * self.image_size[1]/self.image_size[0]))

        self.red_sprite = pygame.image.load("image/fusee - Copie.png")
        self.red_sprite = pygame.transform.scale(
            self.red_sprite, (self.screen_size[0] * 0.085, self.screen_size[0] * 0.085 * self.image_size[1]/self.image_size[0]))

        self.sprite_copy = self.sprite.copy()

        self.image_size = self.sprite.get_size()  # taille de l'image final
        self.pos = [pos[0] - (self.image_size[0]/2),
                    pos[1]]  # position de la fusee

        # pour déplacer la fusee en 2 secondes sur la taille x de l'écran
        ...
        self.rect = pygame.rect.Rect(self.pos[0] + self.sprite.get_size()[
                                     0]/5, self.pos[1], self.sprite.get_size()[0] * 3/5, self.sprite.get_size()[1] * 3/4)
    ...
    def afficher(self, screen):
        # update du rect
        self.rect = pygame.Rect.fit(self.rect, (self.pos[0] + self.sprite.get_size()[
                                    0]/5, self.pos[1], self.rect.width, self.rect.height))
